...
class Runner(object):
    ...
    def extract_embeddings(self):
        """
        Function to get embeddings for each entity and relation in the model
        ...
        self.ent_embed = ddict()
        self.rel_embed = ddict()

        splits = ['train', 'valid_head', 'valid_tail', 'test_head', 'test_tail']

        # Gather embeddings for all data items in the split
        for split in splits:
            self.get(split)

        # Store all entity embeddings as pickle
        with open('ent_embeddings.pickle', 'wb') as handle:
            pickle.dump(self.ent_embed, handle, protocol=pickle.HIGHEST_PROTOCOL)

        # Store all relation embeddings as pickle
        with open('rel_embeddings.pickle', 'wb') as handle:
            pickle.dump(self.rel_embed, handle, protocol=pickle.HIGHEST_PROTOCOL)

    def get(self, split):
        with torch.no_grad():
            batch_iter = iter(self.data_iter[split])
